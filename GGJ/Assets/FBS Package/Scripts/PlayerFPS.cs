...
public class PlayerFPS : MonoBehaviour {
	...
	[Range(1,30)]public float speedMove = 10;
	[Range(0.1f,0.5f)]public float smoothMove = 0.1f;
	...
	[Range(0,10)] public float w_damage = 5;
    ...
    private EnemySettingsScript EnemyHit = new EnemySettingsScript();
	...
	void ShootRay()
	{
		...
		RaycastHit hit;
        Rigidbody bulletInstance = Instantiate(bullet, bullet_spawner.position, Quaternion.identity);
        bulletInstance.velocity = pointShoot.forward * maxSpeed;
        ...
        if (Physics.Raycast(ray, out hit, weapon.range))
		{
			if(hit.collider.gameObject.GetComponent<Rigidbody>())
			{
                ...
                {
                    //#####################Tarek's edits is that the enemy is attacked by the Weapon damage (w_damage)######################//
                    hit.collider.gameObject.GetComponent<EnemySettingsScript>().GotMeMF(w_damage);
                }
                else
                {
